...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	userSecurityEventService := service.NewUserSecurityEventService(db)
	authHandler := handler.ProvideAuthHandler(configConfig, authService, userService, settingService, promoService, redeemService, totpService, userAttributeService, userSecurityEventService)
	...
	serverLoadService := service.ProvideServerLoadService(db, redisClient, configConfig)
	opsHandler := handler.ProvideOpsHandler(opsService, serverLoadService)
	userRiskService := service.NewUserRiskService(db, concurrencyService)
	userRiskHandler := admin.NewUserRiskHandler(userRiskService)
	...
	errorPassthroughHandler := admin.NewErrorPassthroughHandler(errorPassthroughService, settingService)
	...
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, adminUserHandler, groupHandler, accountHandler, adminAnnouncementHandler, dataManagementHandler, backupHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, proxyHandler, adminRedeemHandler, promoHandler, settingHandler, opsHandler, userRiskHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, errorPassthroughHandler, tlsFingerprintProfileHandler, adminAPIKeyHandler, scheduledTestHandler, channelHandler, channelMonitorHandler, channelMonitorRequestTemplateHandler, contentModerationHandler, paymentHandler, affiliateHandler, complianceHandler)
