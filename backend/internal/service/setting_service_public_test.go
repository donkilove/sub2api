...
func TestSettingService_GetPublicSettings_ExposesUniFedHideEmailRegisterUI(t *testing.T) {
	repo := &settingPublicRepoStub{
		values: map[string]string{
			SettingKeyUniFedConnectHideEmailRegisterUI: "true",
		},
	}
	svc := NewSettingService(repo, &config.Config{})
	...
	settings, err := svc.GetPublicSettings(context.Background())
	require.NoError(t, err)
	require.True(t, settings.UniFedHideEmailRegisterUI)
}
