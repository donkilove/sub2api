    ...
    <div class="space-y-6">
      <div class="text-center">
        <h2 class="text-2xl font-bold text-gray-900 dark:text-white">
          {{ t('auth.unifed.callbackTitle') }}
        </h2>
        <p class="mt-2 text-sm text-gray-500 dark:text-dark-400">
          {{ isProcessing ? t('auth.unifed.callbackProcessing') : t('auth.unifed.callbackHint') }}
        </p>
      </div>
        ...
        >
          ...
          <template v-if="needsInvitation">
            <p class="text-sm text-gray-700 dark:text-gray-300">
              {{ t('auth.unifed.invitationRequired') }}
            </p>
            <div>
              <input
            ...
            >
              {{ isSubmitting ? t('auth.unifed.completing') : t('auth.unifed.completeRegistration') }}
            </button>
          </template>
          ...
          <template v-else-if="needsBindLogin">
            ...
            <div class="space-y-3">
              <input
                ...
                data-testid="unifed-bind-login-email"
                type="email"
                class="input w-full"
              ...
              <input
                ...
                data-testid="unifed-bind-login-password"
                type="password"
                class="input w-full"
              ...
              />
              <button
                data-testid="unifed-bind-login-submit"
                class="btn btn-primary w-full"
          ...
          <template v-else-if="needsTotpChallenge">
            ...
            <div class="space-y-3">
              <input
                v-model="totpCode"
                data-testid="unifed-bind-login-totp"
                type="text"
                inputmode="numeric"
                maxlength="6"
                class="input w-full"
              ...
              />
              <button
                data-testid="unifed-bind-login-totp-submit"
                class="btn btn-primary w-full"
...
const providerName = 'Universe Federation'
...
type UniFedPendingActionResponse = PendingOAuthExchangeResponse & {
  step?: string
  intent?: string
  email?: string
...
function extractPendingAccountEmail(completion: UniFedPendingActionResponse): string {
  return (
...
function resolvePendingAccountAction(
  completion: UniFedPendingActionResponse
...
function applyPendingAccountAction(completion: UniFedPendingActionResponse) {
...
function applyTotpChallenge(completion: UniFedPendingActionResponse): boolean {
...
async function finalizeCompletion(completion: PendingOAuthExchangeResponse, redirect: string) {
  ...
  if (!isOAuthLoginCompletion(completion)) {
    throw new Error(t('auth.unifed.callbackMissingToken'))
  }
...
async function finalizePendingAccountResponse(completion: UniFedPendingActionResponse) {
...
async function handleSubmitInvitation() {
  ...
  try {
    ...
    const completion: UniFedPendingActionResponse = legacyPendingOAuthToken.value
      ? (
          await apiClient.post<UniFedPendingActionResponse>('/auth/oauth/unifed/complete-registration', {
      ...
      : affCode
        ? await completeUniFedOAuthRegistration(invitationCode.value.trim(), decision, affCode)
        : await completeUniFedOAuthRegistration(invitationCode.value.trim(), decision)
    await finalizePendingAccountResponse(completion)
  } catch (e: unknown) {
    const err = e as { message?: string; response?: { data?: { message?: string } } }
    invitationError.value =
      err.response?.data?.message || err.message || t('auth.unifed.completeRegistrationFailed')
  } finally {
...
async function handleContinueLogin() {
  isSubmitting.value = true
  try {
    const completion = await exchangePendingOAuthCompletion(currentAdoptionDecision()) as UniFedPendingActionResponse
    await finalizePendingAccountResponse(completion)
  } catch (e: unknown) {
...
async function handleCreateAccount(payload: PendingOAuthCreateAccountPayload) {
  ...
  isSubmitting.value = true
  try {
    const { data } = await apiClient.post<UniFedPendingActionResponse>('/auth/oauth/pending/create-account', {
      email: payload.email,
...
async function handleBindLogin() {
  ...
  isSubmitting.value = true
  try {
    const { data } = await apiClient.post<UniFedPendingActionResponse>('/auth/oauth/pending/bind-login', {
      email,
      password,
      ...serializeAdoptionDecision(currentAdoptionDecision())
    })
...
onMounted(async () => {
  ...
  try {
    ...
    const completion = await exchangePendingOAuthCompletion() as UniFedPendingActionResponse
    ...
    if (applyTotpChallenge(completion as UniFedPendingActionResponse)) {
      persistPendingAuthSession(completionRedirect)
      return
    }

    applyPendingAccountAction(completion as UniFedPendingActionResponse)
    if (pendingAccountAction.value !== 'none') {
      isProcessing.value = false
      persistPendingAuthSession(completionRedirect)
      return
